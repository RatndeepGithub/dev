...
if ( isset( $_POST['ced_amazon_options_nonce'] ) && wp_verify_nonce( sanitize_text_field( $_POST['ced_amazon_options_nonce'] ), 'ced_amazon_options_page_nonce' ) ) {
	if ( isset( $_POST['amazon_options'] ) ) {
		...
		wp_safe_redirect( admin_url() . $ced_base_uri . '&section=setup-amazon&part=wizard-settings&user_id=' . $user_id . '&seller_id=' . $seller_id );
	...
	<form  method="post" >
					...
					?>
						<tr>
							...
							<td class="forminp forminp-select">
								<?php
								if ( 'external_product_id_type' !== $opt_key ) {
		...
		<div class="wc-actions">
			...
			<button style="float: right;" type="submit" class="components-button is-primary amazon_options_save_button button-next" name="amazon_options" ><?php echo esc_html__( 'Save and continue', 'amazon-for-woocommerce' ); ?></button>
				<a style="float: right;" type="button" id="ced_amazon_continue_wizard_button" data-attr='2' href="<?php echo esc_url( admin_url( $ced_base_uri . '&section=setup-amazon&part=wizard-settings&user_id=' . $user_id . '&seller_id=' . $seller_id ) ); ?>"
